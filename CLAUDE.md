...
- `lib/session.tsx` — `SessionProvider`/`useSession()` over real Supabase Auth (SPEC 04). It no longer knows anything about scores: **saving a score does not consult the session**, and `av_scores` no longer exists. Resolving the user (`getUser()` on mount, `onAuthStateChange`) also reads `avatar_url` from `public.profiles` and adds it to `SessionUser` as `avatarUrl` (SPEC 13) — `name` itself is untouched, still resolved from `user_metadata`.
- **`public.profiles`** (SPEC 13): `id` (FK to `auth.users`, PK), `name`, `avatar_url`, `created_at`. Populated only by the trigger `on_auth_user_created` → `handle_new_user()` (`SECURITY DEFINER`, needed because it runs before the new user has its own RLS grants) on `auth.users` insert — nothing in the app inserts into `profiles` directly, and there is no `insert` RLS policy for it. RLS: `select` open to everyone, `update` only where `auth.uid() = id`. Exists to give SPEC 14's OAuth avatar a column to land in; `profiles.name` is populated but not read anywhere yet.
...
- `lib/games/rana.ts` (`rana`) es el quinto juego real y, como `snake`, el primero de su tanda **sin** `game.js` de referencia: la mecánica (Frogger sin motor de físicas) se diseñó al escribir `specs/game-jam/rana/01-rana-game.md`. Es el primero en combinar dos modelos de movimiento a la vez: los carriles de carretera y de río avanzan con la misma **cinemática simple** que ya usa la bola de `bloques` (`x += dir · speed · speedMultiplier · dt`, sin resolución de colisiones física), mientras la rana se mueve en **saltos discretos de una celda** (grid puro) por pulsación de `↑ ↓ ← →`/WASD, con reencaje a grilla y un cooldown de 120 ms que filtra el auto-repeat del teclado. Reusa `.cover-rana` (sin usar desde SPEC 01) y el color `green` que ya tiene `snake` — el `check` de `color` no exige unicidad. El campo `touch` de `registry.ts` (obligatorio desde SPEC 12) se llena aquí con las mismas cuatro flechas que `snake`: el TouchPad de la pantalla de juego es infraestructura genérica de SPEC 11/12, no algo que agregue `mobile_porter` por juego — sus corridas siguen acotadas a las otras seis rutas.
- `rana` es el segundo juego con **audio** (SPEC 02 de game-jam/rana) y el primero que lo **sintetiza con Web Audio API** en vez de portar un asset: no hay ningún `game.js` de Frogger en `references/`, así que tampoco hay ningún MP3 original que portar como sí tuvo `bloques` en SPEC 09. Un `AudioContext` perezoso se crea dentro de `start()` en el primer `beep()`, nunca a nivel de módulo. `loseLife(cause)` toca un beep distinto según `"atropello"` (coche) o `"ahogo"` (todo lo demás: río sin tronco, tronco que saca del canvas, o aterrizar fuera de un nenúfar); ocupar un nenúfar y completar nivel encadenan notas con `setTimeout` porque `beep()` siempre arranca en `audioCtx.currentTime`, sin parámetro de retraso. Reusa la misma clave global `localStorage["av_muted"]` de `bloques` — silenciar uno silencia el otro, es preferencia de plataforma — y los mismos campos opcionales `GameHandle.setMuted?`/`GameEntry.sound?` de SPEC 09, sin tocar sus tipos.
- `rana` gana un segundo tipo de obstáculo de río (SPEC 03 de game-jam/rana): dos de los cinco carriles (filas 3 y 5) son `kind: "turtle"` en vez de `"log"` — cada tortuga cicla sola entre `up`/`warning`/`down` (3000/800/1500 ms, fase inicial aleatoria por obstáculo) y solo sostiene a la rana fuera de `down`, mismo camino de "ahogo" que río sin tronco. `RiverLane`/`RiverObstacle` extienden `Lane` sin tocar los carriles de carretera. Y el multiplicador de velocidad de nivel deja de tener techo — se quita el `Math.min(2.5, ...)` de la fórmula de SPEC 01 — porque es una plataforma de puntuación más alta y un techo fijo volvía "más de lo mismo" a los niveles altos.
- `app/juego/[id]/jugar/client.tsx` — the play screen. Reads `GAMES[id]`; keeps the whole `GameState` in one `useState` and renders NIVEL/LÍNEAS only when that field is defined. **Pause is the platform's alone**: a `paused` prop reaches the game through `handle.setPaused()`, and no game ports its own pause key. Game over opens the modal from `onGameOver` and writes to `public.scores`. Its `TouchPad` (only under `@media (pointer: coarse)`) is the **MK-II gamepad** of SPEC 12 (`references/gamepad-assets/`): `.gp` shell, `.dp` D-pad with the gem hub only when the game declares all four arrows (`bloques` gets `←` `→` in a row), and the action as a round magenta `A` with its `hint` ("Disparar", "Soltar") — no `B`, since no game uses it.
...
- **Manual one-time step (SPEC 14, OAuth social)**: create an OAuth app in Google Cloud Console and one in GitHub Developer Settings, redirect URI `https://<project>.supabase.co/auth/v1/callback` for both, then load each client ID/secret into the Supabase dashboard under Authentication → Providers → Google/GitHub. `app/auth/callback/route.ts` handles the code→session exchange for both providers through one shared route; `lib/session.tsx`'s `signInWithOAuth(provider)` is what the GOOGLE/GITHUB buttons on `/iniciar-sesion` call.
